...
public class DrinkVendingMachine implements VendingMachine<Drink> {
    ...
    /** お客さんの待ち行列 */
    private final BlockingQueue<Customer> customerQueue = Queues.newLinkedBlockingQueue();

    /** 投入されたコインが金庫に入るまでの間、一時置き場として利用する */
    ...
    private static class Exit implements Operation {
        ...
        /**
         * コンストラクタ
         * @param performBeforeExit 終了前に実行される操作
         */
    ...
    private class SelectProduct implements Operation {
        ...
        private void move( Wallet dest, Wallet src, Yen moveAmount) {
            ...
            checkState( workWallet.getTotalAmount().compareTo( moveAmount) == 0, "workWallet=%s, coins=%s, moveAmount=%s, dest=%s, src=%s",
                workWallet, coins, moveAmount, dest, src);
